...
def bark_notify(config: dict[str, Any], title: str, body: str, icon: str = "", url: str = "") -> None:
...
def run_creator(config: dict[str, Any], config_dir: Path, creator_config: dict[str, Any]) -> list[dict[str, Any]]:
    ...
    for post in posts:
        try:
            ...
            records.append({"status": "detail-failed", "post_id": post.post_id, "post_title": post.title, "post_url": post.url, "error": str(exc)})
            continue

        if not candidates:
            records.append({"status": "no-files", "post_id": post.post_id, "post_title": title, "post_url": post.url})
            continue
        ...
        post_dir = post_output_dir(download_dir, creator_name, post.publish_time, title, post.post_id)
